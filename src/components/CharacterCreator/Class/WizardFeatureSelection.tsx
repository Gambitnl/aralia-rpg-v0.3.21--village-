...
import { Spell, Class as CharClass } from '../../../types'; // Path relative to src/components/CharacterCreator/Class/
...
const WizardFeatureSelection: React.FC<WizardFeatureSelectionProps> = ({
  spellcastingInfo,
  allSpells,
  onWizardFeaturesSelect,
  onBack,
}) => {
  const [selectedCantripIds, setSelectedCantripIds] = useState<Set<string>>(
    new Set(),
  );
  ...
  const [selectedSpellL1Ids, setSelectedSpellL1Ids] = useState<Set<string>>(
    new Set(),
  );
  ...
  const handleSubmit = () => {
    if (
      ...
      const cantrips = Array.from(selectedCantripIds).map(
        (id) => allSpells[id],
      );
      const spellsL1 = Array.from(selectedSpellL1Ids).map(
        (id) => allSpells[id],
      );
    ...
    <div>
      ...
      <div className="mb-6">
        <h3 className="text-xl text-amber-300 mb-2">
          Select {spellcastingInfo.knownCantrips} Cantrips
        </h3>
            ...
            <label
              ...
              className={`p-2 rounded-md cursor-pointer transition-colors ${
                selectedCantripIds.has(spell.id)
                  ? 'bg-sky-600 ring-1 ring-sky-400'
              ...
              <input
                ...
                onChange={() =>
                  toggleSelection(
                    ...
                    spellcastingInfo.knownCantrips
                  )
                }
                disabled={
                  !selectedCantripIds.has(spell.id) &&
        ...
        <p className="text-xs text-gray-400 mt-1">
          Selected: {selectedCantripIds.size}/{spellcastingInfo.knownCantrips}
        </p>
      </div>
      ...
      <div className="mb-6">
        <h3 className="text-xl text-amber-300 mb-2">
          Select {spellcastingInfo.knownSpellsL1} Level 1 Spells for your
          Spellbook
        </h3>
        <p className="text-xs text-gray-400 mb-2">
          (These will be your initially prepared spells.)
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              ...
              <input
                ...
                onChange={() =>
                  toggleSelection(
                    spell.id,
                    selectedSpellL1Ids,
                    setSelectedSpellL1Ids,
                    spellcastingInfo.knownSpellsL1
                  )
                }
                disabled={
                  !selectedSpellL1Ids.has(spell.id) &&
        ...
        <p className="text-xs text-gray-400 mt-1">
          Selected: {selectedSpellL1Ids.size}/{spellcastingInfo.knownSpellsL1}
        </p>
      </div>
      ...
      <div className="flex gap-4 mt-6">
        <button
          onClick={onBack}
          className="w-1/2 bg-gray-600 hover:bg-gray-500 text-white font-semibold py-2 px-4 rounded-lg shadow"
